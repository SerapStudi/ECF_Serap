<?php   
    ...
    if(!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['new_password_retype'])){
        ...
        $check_password  = $bdd->prepare('SELECT password FROM utilisateurs WHERE acces = :acces');
        ...
        if(password_verify($current_password, $data_password['password']))
        {
            ...
            if($new_password === $new_password_retype){
                ...
                $update = $bdd->prepare('UPDATE utilisateurs SET password = :password WHERE acces = :acces');
                $update->execute(array(
                    "password" => $new_password,
                    "acces" => $_SESSION['user']
                ));
                // On redirige
                header('Location: 10_landing.php?err=success_password');
                die();
            }
        }
        else{
            header('Location: 10_landing.php?err=current_password');
            die();
        }
